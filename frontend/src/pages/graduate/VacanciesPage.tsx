      ...
      <Card className="overflow-hidden">
        <CardContent className="p-5 sm:p-6">
          <div className="mb-6 grid grid-cols-1 gap-3 lg:grid-cols-[minmax(0,1fr)_220px_220px]">
            ...
            <select className={fieldClass} value={isActiveFilter} onChange={(event) => setIsActiveFilter(event.target.value as ActiveFilter)}>
              <option value="">{t('common.all')}</option>
              <option value="true">{t('vacancy.active')}</option>
              <option value="false">{t('vacancy.inactive')}</option>
            </select>

            <select className={fieldClass} value={createdSort} onChange={(event) => setCreatedSort(event.target.value as CreatedSort)}>
