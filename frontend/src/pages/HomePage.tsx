...
const isEmploymentStatsEntry = (value: unknown): value is EmploymentStatsEntry => {
  if (!value || typeof value !== "object") return false;
  const item = value as Partial<EmploymentStatsEntry>;
  return typeof item.total === "number" && typeof item.employed === "number";
};
...
const HomePage: React.FC = () => {
  ...
  const [isHeroImageAvailable, setIsHeroImageAvailable] = useState(
    Boolean(heroImage),
  );
  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        Object.entries(data).forEach(([key, value]) => {
          if (key !== "meta" && isEmploymentStatsEntry(value)) {
            total += value.total;
        ...
        setEmploymentRate(
          surveyed > 0 ? Math.round((employed / surveyed) * 100) : 0,
        );
    ...
    const fetchPartners = async () => {
      try {
        ...
        const activePartners = getListResults<Partner>(res.data).filter(
          (partner) => partner.is_active !== false,
        );
        setPartners(activePartners);
        setTotalPartners(
          getPaginationMeta<Partner>(res.data, 1, PARTNERS_LIMIT).count,
        );
      } catch (error) {
        console.error("Failed to load partners:", error);
        setPartners([]);
        setTotalPartners(null);
      }
    };
  ...
  const visiblePartners = useMemo(
    () => partners.slice(0, PARTNERS_LIMIT),
    [partners],
  );
  const hiddenPartnersCount =
    totalPartners !== null
      ? Math.max(totalPartners - visiblePartners.length, 0)
      : 0;
  ...
  return (
    <div className="animate-fade-in bg-white">
            ...
            {statsCards.map((item) => (
              ...
              >
                <CardContent className="relative p-7 md:p-8 text-center">
                  ...
                  <div
                    className={`mx-auto mb-5 flex h-16 w-16 items-center justify-center rounded-3xl ${item.bg} ${item.accent} ring-1 ${item.ring} group-hover:scale-105 transition-transform`}
                  >
                    {item.icon}
                  </div>
                  <h3
                    className={`text-4xl md:text-5xl font-bold tracking-tight ${item.accent}`}
                  >
                    {item.value}
                  </h3>
          ...
          <div className="w-full rounded-[2rem] border border-primary-100 bg-gradient-to-br from-primary-50 via-white to-accent-50 p-5 md:p-8 shadow-sm">
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 md:gap-5">
              {programCards.map((item) => (
                <div
                  key={item.code}
                  className="rounded-3xl bg-white p-6 border border-gray-100 shadow-sm"
                >
                  <div className="mb-5 flex items-center justify-between gap-4">
                    <div>
                      ...
                      <p className="mt-2 text-3xl md:text-4xl font-bold text-primary-700">
                        {item.code}
                      </p>
                    </div>
              ...
              <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-5">
                ...
                <div className="flex flex-wrap gap-2 md:justify-end">
                  {[
                    ...
                    <span
                      key={item}
                      className="rounded-full bg-primary-50 px-3 py-1.5 text-xs font-semibold text-primary-700 ring-1 ring-primary-100"
                    >
                      {item}
                    </span>
                  ))}
      ...
      {!user && (
        <section className={hasPartners ? 'bg-gray-50 pb-16' : 'bg-gray-50 py-16'}>
